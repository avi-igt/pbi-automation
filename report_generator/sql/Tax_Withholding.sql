SELECT
  ...
  PAYMENT_PRODUCT.PRODUCT_ID as "Product ID",
  BOA_PS.PS_PAYMENT.PAYMENT_INSTRUMENT_ID as "Payment Check Number",
  COALESCE(TRIM(CLAIMANT_PLAYER.FIRST_NAME),'') as "Payee First Name",
  COALESCE(TRIM(CLAIMANT_PLAYER.LAST_NAME),'') as "Payee Last Name",
  sum(BOA_PS.PS_PAYMENT.PAYMENT_GROSS_AMOUNT) as "Total Gross Amount",
  SUM(PAYMENT_DEDUCTIONS.FED_TAX) as "Total Fed Tax",
  SUM(PAYMENT_DEDUCTIONS.STATE_TAX) as "Total State Tax"
FROM
...
where
    ...
    and prod.product_oid = t.product_oid
     ...
     LEFT OUTER JOIN BOA_PS.PS_CLAIMANT ON BOA_PS.PS_CLAIM.CLAIM_OID=BOA_PS.PS_CLAIMANT.CLAIM_OID
...
WHERE
  (
   ...
   AND
   ESTYPE_PMT_STATUS.DISPLAY_TEXT  IN  ( 'Sent To Payment'  )
  )
GROUP BY
  date(BOA_PS.PS_PAYMENT.PAID_DATETIME ), 
  PAYMENT_PRODUCT.PRODUCT_ID, 
